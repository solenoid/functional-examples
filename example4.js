...
// var res = lo.take(lo.map(lo.map(lo.map(EXAMPLE_DATA, marginCalc), ctrCalc), dateCalc), 6);
// using idomatic chaining
var res = lo.chain(EXAMPLE_DATA).map(marginCalc).map(ctrCalc).map(dateCalc).take(6).value();
console.log(res);
